...
/**
 * Apply all enabled tag-rules against the underlying data tables and upsert
 * auto-assignments. Safe to re-run — uses onConflict upsert.
 */
export async function applyTagRules(
  ...
  const byType: Record<TaggableEntity, TagRule[]> = {
    keyword: [],
    post: [],
    campaign: [],
    source: [],
  ...
  if (byType.keyword.length > 0) {
    ...
    await paginate<{ query: string }>("search_keywords", "query", (rows) => {
    ...
    await paginate<{
      ...
      (rows) => {
        for (const row of rows) {
          ...
          if (seen.has(key)) continue;
          seen.add(key);
          const target = `${row.campaign_name} ${row.ad_group ?? ""} ${row.keyword ?? ""}`;
          for (const rule of byType.campaign) {
            if (ruleMatches(rule, target)) emit(rule, key);
          }
        }
      }
    );
    ...
    await paginate<{
      ...
      (rows) => {
        for (const row of rows) {
          ...
          if (seen.has(key)) continue;
          seen.add(key);
          const target = `${row.channel} ${row.source ?? ""} ${row.medium ?? ""}`;
  ...
  if (toUpsert.length === 0) {
    return { applied: 0, rules_evaluated: rules?.length ?? 0 };
  }
  ...
  return { applied: inserted, rules_evaluated: rules?.length ?? 0 };
}
