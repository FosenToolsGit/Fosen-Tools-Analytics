...
| 19. sept (sidevekt) | **Forsiden veier 20,7 MB, og 14,8 av dem er merkelogoer.** 110 logoforespørsler fra **55 unike filer — hver hentes to ganger** (samme URL, to `<img>` ved ~370 og ~876 ms; karusellen rendres trolig dobbelt og bommer på hurtigbufferen). 🔴 **20 av 50 logoer er PNG-er pakket i SVG med null `<path>`, til sammen 6,5 MB** — `The-Bone.svg` er **3 260 kB** (2 445 kB base64-PNG inni, største enkeltressurs på nettstedet), Emhart 700, Scell-It 679, Red-Rooster 216, Milwaukee 207, Bondhus 178, Stahlwille 168, Zarges 150 osv. De 30 ekte vektorene veier 701 kB til sammen (Wera 3 kB, Hellberg 2 kB). **Samme mønster som Bahco-logoen i august.** `/snapon` veier 17,1 MB med 5,50 s LCP og score 44 — tregeste sidetype, og den som konverterer best på merkesøk; banneret er 1 333 kB etter 10000×2500-konvensjonen. **Ubrukt JS er stort sett tredjepart:** tre separate Google-tagger (gtag×2 + gtm = 217 kB ubrukt), Facebook 66 kB, Freshworks 35, Cookiebot 31, og vår egen `web-defered-4.0.js` 101 av 126 kB. Ubrukt CSS er vår egen: `FosenTools.css` 61 av 68 kB (90 %). **Ærlig om verdien:** ekte besøkende merker det ikke (CrUX gir LCP 0,65/0,92 s), så det er ikke en rangeringssak — men det er en reell datakostnad, en sannsynlig bidragsyter til desktop-CLS på 0,30, og uvanlig billig å rette. Rapport: `~/Desktop/FT-logoer-og-sidevekt.html`. Memory: `project_sidevekt_logoer`. |
