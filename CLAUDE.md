...
- **Developer Token:** Basic Access **godkjent** (bekreftet i drift 17. aug 2026 — Keyword Planner leverer ekte volum, f.eks. «verktøyvogn» 3600/mnd)
...
**Leverandør-videoer fra mediebank (Milwaukee o.l.) — onsdag-poster:**
- **Onsdag i ferieperioder (og ellers ved behov) = leverandør-produktvideo fra mediebanken + våre captions.** Leverandøren (f.eks. Milwaukee via TTI-mediebanken) lager ferdige 9×16-produktvideoer; vi lager kun captions. Format som torsdags-tipsene: **IG + FB (ingen LinkedIn — reel)**, IG bruker «Søk varenr. {FT-sku} på fosen-tools.no» (IG har ikke klikkbar bio-lenke per produkt), **maks 5 hashtags**, verifiserte spec (aldri finn på), **ingen pris med mindre bedt om**. FB kan ha egen klikkbar lenke per produkt med `utm_content={varenr}`. Captions leveres som `captions.html` med kopi-knapper (dark FT-stil).
- **Regel: lag KUN captions for produkter FT faktisk fører OG har på lager.** Sjekk alltid mot fosen-tools.no først (Multicase-søket er JS-rendret → curl ser det ikke; bruk **sitemap** + Googlebot-UA, jf. quirk #14). FT-status: `stock` (fører + lager → lag captions), `oos` (fører, utsolgt → vent på lager), `no`/`na` (fører ikke → hopp over / «vurder å ta inn»). Kort-URL `fosen-tools.no/milwaukee/{varenr}` 301-redirecter til full slug og lander på produktsiden (UTM overlever). Modellkode ≠ FT-slug (f.eks. «M18 JS2000» ligger som «startbooster-m18-2000a»).
- **Mediebank-assets merket «(1) Pre-launch»** er under embargo til offisiell lansering — ikke post før lanseringsdato (om FT fører produktet, men det er utsolgt, henger det ofte sammen med pre-launch).
- **Milwaukee video-register:** `~/Desktop/Milwaukee-video-register/` — søkbar/filtrerbar HTML-oversikt over alle mediebank-produktvideoer med FT-status. Kilde: TTI «Milwaukee Marketing Media External / Videos / {år} / {kvartal}». `videos.json` = data, `build.mjs` = renderer `index.html`, `seed-*.mjs` per kvartal. **Ny leveranse:** Adrian limer inn mediebank-HTML → krysssjekk hver mot fosen-tools.no → legg inn i `videos.json` → kjør `build.mjs`. Q1-2026 inne (67 asseter); Q2–Q4 + 2024/2025 kommer. Full doc: `memory/project_leverandor_video_mediebank.md`.
...
- `Fosen-Tools GA4 (web) purchase` er **primary** med verdier fra GA4 (ekte transaksjonsverdi)
- `Fosen-Tools GA4 (web) form_submit` er **primary for leads** med telling **«Én konvertering»** (satt 17. aug 2026 — før det talte «Alle», og phantom-events ga 29 «leads» på én dag)
- `Kontaktoss skjema` (AW-taggen) ble **fjernet 17. aug 2026** — den hadde sluttet å fyre (0 konv., «Handling kreves») og var redundant mot GA4-importen. Eldre notater som sier den er primary er utdatert.
- `klikk_kontakt_oss` er fjernet som primary (var eneste primary tidligere — drev Pmax blindt)
- **Ikke «fiks» Feilkonfigurert-merkene** på Legg i handlekurven / Start betalingsprosessen / Sidevisning: de er sekundær-only med vilje (begin_checkout ≠ lead). Å gjøre dem primære ville satt budgivning mot handlekurv-klikk. NB: «Endre målet»-dialogen i Google Ads **pre-stager** slike promoteringer — les forhåndsvisningen («X er endret») før Lagre.
...
- Developer token har Basic Access (godkjent en gang mellom april og august 2026; bekreftet 17. aug). KeywordPlanIdeaService er i drift — `/innsikt/vekst` og rapport-arkene får ekte volum
...
1. **Nettside-script** pusher `ft_contact_form_success` til dataLayer ved iframe-reload — **men kun etter faktisk interaksjon med skjemaet** (nytt script 17. aug 2026: blur-lytter + 300ms-polling på `document.activeElement === iframe`). Den gamle rene reload-heuristikken telte phantom-reloads som innsendinger — 29 «leads» 29. juli med null tickets i Freshdesk — og kunne bytte ut skjemaet med falsk takke-boks for ekte besøkende. Interaksjonskravet styrer BÅDE eventet og takke-boksen.
2. **GTM-trigger** `Custom - FT Contact form success` lytter på `ft_contact_form_success`
3. **GA4-event `form_submit`** importeres til Google Ads som primary lead-konvertering med telling «Én per klikk». (AW-taggen «Kontaktoss skjema» ble fjernet 17. aug — død og redundant.)
...
16. **GA4 har en bot-flom i høsten 2025 — korriger alltid før sammenligning mot i fjor.** 1. sept–30. nov 2025 var kanalen «Unassigned» 44–60 % av all trafikk på fosen-tools.no (sept 60 %, okt 46 %, nov 44 %), med 0,7 % nøkkelhendelser og 3,8 % engasjement — 6 552 sesjoner fra land «(not set)» på desktop uten landingsside. **Nettstedet var ikke nede og trafikken var ikke mennesker.** Vår egen database hjelper ikke: `analytics_metrics` starter i januar 2026, så høsten 2025 finnes bare i GA4. Bruk **`npm run aar-mot-aar -- --fra 2026-10-01 --til 2026-10-31`** ([scripts/aar-mot-aar.mjs](scripts/aar-mot-aar.mjs)) — den henter begge år, trekker fra Unassigned i fjorårsvinduet, advarer når perioden overlapper, og viser rått mot korrigert side om side. Effekten er stor: september 2026 mot 2025 gir **−68,5 % rått og +3,1 % korrigert**. Vinduet står som `FORURENSET` øverst i scriptet. Steg 4 i `/mandagsokt`.
...
| 9. juni | **Nyhetsbrev-byggeren — lærdommer + regler.** IKKE auto-schedule via Mailchimp API (footer rendrer smal i Gmail → push som utkast, schedule manuelt i Mailchimp UI). Wizard auto-save-bug fikset. Regler låst: **aldri finn på claims** (ikke frister/tall uten kilde), **korte titler/undertitler**, **ingen emojis i nyhetsbrev**. **Reels kun på IG+FB** (ikke LinkedIn; LinkedIn = organisk B2B), reel-format 1080×1920 default. Memory: `project_nyhetsbrev_lardommer_9juni`, `feedback_reels_kun_ig_fb`, `feedback_nyhetsbrev_aldri_paafunn`. |
| 12. juni | **Jubileums-rollout + SEO coverage-audit.** Bygde 10 SoMe-utkast (Playwright HTML→PNG, sendt til Claude Design for finpuss), jubileums-e-post til 71xx-segment (Mailchimp Custom HTML, leverandør-logoer i grid, HTML-entiteter for æøå), 2 Meta-teaser-varianter (drone + leverandører, 4 formater m/ 9:16 safe-zone), oppdaterte jubileums-siden m/ Eriks tekst + Zweibrüder-link-fiks (`/ledlenser`→`/zweibruder`). **SEO coverage-audit** (`scripts/_tmp-seo-coverage-audit.mjs`) av hele sitemap (4 776 URLer, Googlebot-UA): **100% sunne (200 OK + indekserbare)**. Fant rot-årsak til GSC-oppblåsing: **tom `User-agent: Googlebot / Disallow:`-blokk i robots.txt nuller ut all crawl-beskyttelse** → Google crawler 3 929 noindex + 10 855 «gjennomsøkt-ikke-indeksert» junk-URLer (søk/filter). Ferdig Multicase-melding på `~/Desktop/multicase-robots-txt-fix.html` — **send mandag 15. juni** (Multicase support stengt 12. juni). Memory: `feedback_robots_txt_googlebot_hull.md`. |
| 15. juni | **Mandag: nyhetsbrev + brief-fixer + Meta-rapport.** Tirsdags-nyhetsbrev Knipex valgt + klart (levert som HTML til utklipp, ikke API-push — schedules manuelt tirsdag 11:00). Nye nyhetsbrev-standarder kode-håndhevet: topp-badge «NYHETSBREV» default, **pris kun ved rabatt**, **FT-artikkelnr** (ikke produsentens modellnr), mobil-stacking (`mceColumn`). **Møtebrief-fix: `begin_checkout` ≠ lead** (var feilaktig talt som lead 4 steder → nå kun form_submit/kontakt = lead; begin_checkout = egen pipeline-post). Meta-annonse-rapport til mandagsmøtet (`~/Desktop/meta-resultater.html`). 4-generasjoners jubileumsbilde mottatt (4. person ubekreftet — må verifiseres). Memory: `feedback_begin_checkout_ikke_lead`. |
| 16. juni | **Jubileums-SoMe-poster (partner + nedtelling) + SEO produkt-gap-register.** Jubileums-poster bygget som HTML→PNG via Playwright (editorial-maler i `~/Desktop/Jubileum-Claude-Design-RESULTAT/jubileum-poster/`, output til `~/Desktop/Jubileum-{X}-FERDIG/`): **Milwaukee** (publisert IG, FB+LI scheduled — RED Evolution Heavy Duty Days, «Milwaukee kommer til jubileet»), **Wera** (klar til 17. juni — foto/logo-forward, stor hvit logo + grønn vegg, «nyheter og innovasjon innen håndverktøy»), **«Én uke igjen»** (T-7 fredag 19. juni — dristig sentrert grafikk-countdown: FT-rød + solstråle-vektor + sirkel-medaljong med Factory Store-bilde + chips; 4:5 ferdig+godkjent, mangler 1:1+captions+ALT). Hver post eget uttrykk. **SEO produkt-gap-register** (`~/Desktop/FT-SEO-produkt-gap-register.xlsx`): Leatherman (3 hull: Free P4/P2/Signal), Milwaukee (on-page OK bekreftet via GSC URL Inspection API — undersider-mulighet, ikke produkt-hull), Pelicase (autoritets-konkurranse, ikke fixbart) + Undersider-byggeliste-ark + Prioritet-kolonne. Multicase-melding om merke×kategori-undersider draftet (`~/Desktop/multicase-merke-kategori-sider.html`). Memory: `project_jubileum_some_poster_produksjon`, `project_seo_produkt_gap_register`, `feedback_merke_kategori_undersider`. |
| 17. juni | **Regel låst: «proff-butikken», ikke «butikken».** Gjelder all FT-jubileums-/butikk-omtale i captions/plakat/nyhetsbrev/SoMe/alt. ✅ «Dagstilbud kun i proff-butikken». Memory: `feedback_proff_butikken_ikke_butikken`. |
...
| 18. sept (kveld) | **Full SEO-gjennomgang av alt som lar seg måle.** To funn bærer resten. **(1) Topp 3 eller ingenting:** 8,2 % klikkrate i topp 3 mot 1,3 % på plass 4–10 — topp 3 er 6 % av visningene og 43 % av klikkene, så å flytte en side fra plass 11 til 9 er nær verdiløst. Enhetssplitt-forklaringen testet og forkastet. **(2) Generiske søk gir ikke klikk:** 72 % av visningene, 32 % av klikkene, plass 21 i snitt; merkesøk er 22 % av visningene og 56 % av klikkene. Bare 6,3 % av søkeordene ga klikk, og 42 søkeord gir halvparten. **To kjernekategorier faller:** verktøyvogner («verktøyvogn» plass 13 → 26) og containere (−45 % visninger, plass 32 → 35, 12,4 % av omsetningen). **Produsentmalen målt:** 29 sider med mal 3,57 % mot 131 uten 2,15 %, og 23 organiske sesjoner per side mot 2,6 på kategorisider. **Kannibalisering:** 107 søkeord med 300+ visninger treffer flere egne sider — 85 751 visninger, 949 klikk. **Fallsikring:** etterspørselen firedoblet (1 982 → 7 540 visninger), plass 64 → 48 — «feil marked»-beslutningen ble tatt på tall som ikke lenger gjelder. Rot: 563 sitemap-sider uten visninger (59 gamle referansesider), 40 selvnøstende URL-er, én rå `.art`-adresse indeksert. 4 308 sider med visninger ligger utenfor sitemap (45 % av klikkene). Bing kjenner 126 av 8 856 sider. AI Assistant er ny egen GA4-kanal. September ligger over juni per dag — problemene er strukturelle, ikke akutte. Rapport: `~/Desktop/FT-analyse-18sept/hovedrapport.html`. **Sesong:** julekalenderklyngen er den eneste der vi både rangerer i topp 5 og blir klikket på — 4,18 % klikkrate i nov 2025 mot 1,8 % ellers, 10 540 visninger og 433 klikk okt–des. Trafikken går til produktsidene, og årstallet står i søket, så 2026-utgavene bør ligge ute nå (443 visninger på plass 6,8 allerede i september). **Hjulskift** er lite og fallende (1 725 → 695 visninger, plass 20 → 27), men toppet uken fra 6. okt i fjor, så kampanjelanseringen treffer. **Sideombyggingene** i aug/sept flyttet posisjon (tang 10,9 → 7,7) men ikke klikk — mål på nytt ~10. okt. **Minnet ryddet:** 217 filer → 138, MEMORY.md fra 217/24,8 kB til 139 linjer/16,4 kB; 89 småfiler slått sammen til 10 temafiler, ingenting mistet. **Dypanalyse:** egen klikkrate-kurve fra 19 420 søkeord-side-par viser at **søketype slår posisjon** — på plass 1 får vi 23,65 % på merkesøk og 1,98 % på generisk, tolv ganger forskjell på samme posisjon. Det avgjør snippet-mot-SERP-spørsmålet: forskjellen er hva som ellers fyller skjermen, ikke metateksten vår. Sitelenker er 31 % av topp-3-visningene og drar snittet fra 7,09 % til 5,00 %. Produsentside slår produktside 2,4× på merkesøk ved samme posisjon. **Sytten måneder:** posisjon 19,84 → 14,02 og klikkrate 1,64 % → 1,83 % — nettstedet er bedre stilt enn for halvannet år siden. Men generisk posisjon gikk 26,2 → 21,3 og ga bare 0,46 % → 0,51 % klikkrate, mens leverandørmerke gikk 1,98 % → 3,07 %. Fem plasser på generiske søk = 0,05 prosentpoeng. Hypotesen om at høsttoppen 2025 var sesongklyngene ble testet og forkastet. **Ingen inntektssignal:** av 385 kjøpte søketermer i 2026 har to konvertert, begge eget merke; organisk kjøpssporing har ikke fyrt siden 20. aug. **Siste lag — kilder vi aldri hadde sett på:** bildesøk er 18 % av synligheten (69 291 visn, 352 klikk, 349 sider finnes bare der); selgeroppføringer gir 4,64 % mot 2,27 % for tekstutdrag, men URL Inspection avkreftet at dekningsgapet er et markupgap — et regnestykke på +3 900 klikk ble forkastet før publisering. Interne søk på nettstedet er merkedominert (snap on, wera, pb swiss), og `vogn m hjul` gir 0 % engasjement på 18 søk. **Handletrakten:** produsentside-landing gir 5,70 % i kurv og 76,7 % videre til kasse, best av alle sidetyper — femte uavhengige bekreftelse. Vognene får trafikken (0,4–2,8 % konvertering), småverktøyet salget (dåpskoffert 17,24 %). Betalt søk har 54 % returnerende, så Brand Search betaler i hovedsak for gjenbesøk. Oslo er større enn Trondheim; Bodø har 73 % nøkkelhendelser på 120 sesjoner. **Bing:** 9 653 sider i indeks, 126 med visninger, 53 % av alt crawlet er 301, 0 stk 404, 334 inngående lenker. **Blindsoner:** PageSpeed 429 uten API-nøkkel, Bings lenke-API tomt. **Datafeil:** GA4 får produktnavn HTML-entitetkodet; sitemapen sist innmeldt 31. aug 2023. **Verktøyvogn-fallet datert:** `/produkter/verktøyvogner` gikk fra 2 966 til 104 visninger i måneden og fra plass 10 til 59. Google byttet hvilken side den svarer med på «verktøyvogn» mellom des 2025 og jan 2026 — til `/skreddersøm/verktøysett/vogn-med-verktøy`, som selv har falt til plass 32. To sider til vokste fram på samme søk (`/produkter/oppbevaring/vogner` og `/oppbevaring/skap`). Fire egne sider konkurrerer nå, alle begravd; ingen er utpekt som svaret. **Ytelse målt i ekte nettleser** (PageSpeed-API-et gir 429 uten nøkkel): serveren er rask (57–194 ms), men hver side sender 791 kB stilark i fire filer (~6 700 regler) og 850–1 640 kB JS. Én render-blokkerende ressurs i head er Google Fonts for Playfair Display + Roboto — **Playfair Display brukes ingen steder**. Elleve tredjepartsdomener. Kritisk CSS bekreftet ikke aktivert. **SERP-en sett med egne øyne 19. sept** (blindsonen lukket — ingen CAPTCHA i ekte nettleser): på «rørtang» ligger 27 sponsede produkter og en AI-oversikt over første organiske treff, og AI-svaret anbefaler «Biltema, Jula og Würth» ved navn. På «verktøyvogn» kommer første organiske treff etter 4 413 tegn med shopping; på «containerlås» etter 3 985. På alle tre er Fosen Tools ikke på siden. **Kontrollen «snap on norge»: vi ER det første organiske treffet** — samme sidestruktur, motsatt utfall, og hele forklaringen på 23,65 % mot 1,98 %. Metatekst på generiske kategorisider kan ikke virke fordi det ikke finnes en plass på skjermen der den blir lest. **Utvidet til 13 SERP-er 19. sept:** betalt innhold starter innen tegn 144–212 på **alle tretten**, snitt 3 800 tegn ned til første organiske treff, ingen unntak; AI-oversikt på bare 2 av 13. Vi er synlige på tre: merkesøket, julekalenderen, snekkerbeltet. **Konkurrentkart:** jula.no 6 treff, biltema.no 5, **tools.no 4, wuerth 4, ahlsell 2** — Tools/Würth/Ahlsell er like tilstede som forbrukerkjedene og er ekte B2B-konkurrenter, noe som korrigerer «Biltema og Jula eier ordene». Containere er en annen bransje (containex, **finn.no**, spacebox, bns-container), og fallsikring er bemannet av spesialister (3M, Certex) — ikke feil marked, men en hardere vei inn. **Egne treff sett i Google (site:-søk):** kategorisidenes metatekst er **god** — bekreftet visuelt («Krympetenger for sikker og presis pressing» osv., spisstang viser utvidbar FAQ), så metarunden er endelig avklart som unødvendig. **Google leser JS-metaen på produsentsider** (`/snapon` vises med riktig tittel og beskrivelse) — lukker saken fra 31. aug: JS-problemet rammer bare Bing og dermed ChatGPT/Copilot. **Produktsidene viser rå engelsk leverandørtekst** i beskrivelsen («Country of Origin USA · Screw Thread Spec…») og titler i versaler med artikkelnummer — men Snap-on er likevel vår beste produktgruppe (2,63 %), så dette er en forbedring, ikke en forklaring. Sitenavnet veksler mellom «Fosen Tools» og «Fosen Tools AS». Memory: `project_konkurrentkart_serp`. **Alt lagret for overlevering:** `~/Desktop/FT-analyse-18sept/STATUS.html` sier hva som er avklart (metarunde avlyst, JS-meta rammer bare Bing, tre hypoteser testet og forkastet), hva som står åpent (feltdata på ytelse krever gratis PageSpeed-nøkkel, lenkeprofil, omsetning per søkeord), hvor rådata og de 31 analysescriptene ligger, og metodene som virker. Memory: `project_seo_analyse_status` er inngangen. Memory: `project_seo_funn_2026` (samlefil), `project_verktoyvogner_faller`, `project_ytelse_malt_i_nettleser`, `project_julekalender_sesongklyngen`, `project_aktuelt_artikkelsider`, `feedback_memory_temafiler`. |
| 19. sept | **SEO-analysen runde to — søkeresultatsiden avgjør, ikke posisjonen.** 62 generiske søk og 12 merkesøk hentet live, 13 målt i ekte nettleser. **Hovedfunnet:** på våre 62 største generiske søk er klikkraten flat uansett plassering — 0,37 % på plass 3–6, 0,23 % på 6–10, 0,41 % på 10–20, 0,22 % forbi 20; til sammen 42 996 visninger og 124 klikk. Vi er alt på side 1 på 22 av dem, og de gir 41 klikk på 90 dager. Rangering på generiske søk er ikke leveransen. **Snap-on er unntaket og forklaringen:** «snap on» gir 13,95 %, «pelicase» 0,99 % — begge med oss som organisk nummer én, samme mal. 12 av 13 målte SERP-er har sponsede produkter innen tegn 200; «snap on» har ingen annonser i det hele tatt, og er det eneste merket uten konkurrerende norsk forhandler på side 1. Wera har 5, Hellberg 6, Milwaukee Packout 3 — alle under 1 %. **Produsentmal-arbeidet bør derfor velge merker etter konkurranse, ikke volum** (målt for 12 av 45 merker). **Containerne er den reneste kontrollen:** vi er nummer to på 6-fots-søkene og ikke på side 1 på 10- og 40-fots; sidene er strukturelt like (590 mot 636 ord, samme mal, samme JSON-LD), men 10-fots-feltet har ni rene containerleverandører. Avlyser «forbedre siden» som plan. **Intentbøtter (13 430 søkeord):** ekte generisk 67 % av visningene og 0,48 %, leverandørmerke 23,9 % og 2,89 %, eget navn 1,0 % og 10,81 %, konkurrentnavn 4,0 % og 0,07 %. «tools» alene er 6 647 visninger og 1 klikk; «tools.no» 1 112 og 0 med forsiden vår på plass 4,5. Å fjerne all støyen flytter site-CTR fra 1,16 % til 1,22 %. **Tre hypoteser testet og forkastet:** bildesøk forklarer ikke de klikkløse søkene (bildeposisjonen er alltid dårligere), bildekarusell gjelder fire av 62 («rissenål» står som miniatyr på side 2), og **engelsk leverandørtekst er marginalt** — 130 tilfeldige produktsider gir 82 % norsk, 2 % engelsk, 12 % uten beskrivelse, så notatet fra 18. sept gjaldt enkeltsider og ikke gruppen. Sidefunn: 16 av 130 mangler beskrivelse, 127 av 130 har H1 i rene versaler. **Fortsatt blokkert:** PageSpeed og CrUX avviser både AI Studio-nøkkelen og OAuth fra tjenestekontoen — det trengs en egen gratis nøkkel fra Google Cloud. Rapport: `~/Desktop/FT-analyse-18sept/runde2.html`, status oppdatert i `STATUS.html`. Memory: `project_serp_avgjor_ikke_posisjon`. |
| 19. sept (forts.) | **Norske forhandlere målt for alle 60 merker — og runde to måtte korrigeres.** Målt i ekte nettleser, ett søk per merke. **Der vi kan vinne er det lite å hente:** 13 merker der vi står i topp 3 gir 14,08 % på 5 158 visninger, men de tre merkene med åpen søkeside der vi mangler (Gigant, Gühring, Norbar) har bare 1 144 visninger til sammen. **77 % av all merkeetterspørsel (38 482 visninger) ligger i «trang søkeside»** — annonser innen tegn 300 og 3–8 norske forhandlere på side 1 — og der får vi 1,59 %. **Posisjon betyr alt på merkesøk, motsatt av generiske:** topp 3 gir 8,80 %, plass 4–10 gir 1,08 %, side 2 gir 0,40 %. Annonser: 0,80 % mot 8,32 % uten. **Korreksjon:** anslaget på 589 klikk fra produsentmal-arbeid var for høyt — snittet på 2,89 % lages av to merker der vi står øverst i en tynn søkeside, og er ikke et nivå Milwaukee eller Wera kan nå. Produsentmalen skal ikke rulles bredt ut; 16 av de 30 sidene som har den ligger i trange søkesider og 5 på merker der etterspørselen ikke er vår. **Milwaukee er 38 % av all merkeetterspørsel** (19 332 visn på 1 459 søkeord); merkenavnet alene er 289. **Packout-klyngen: 3 409 visninger, 9 klikk** — tallfester tiltak 5 og er den største målte merkemuligheten. **Tolv merker der etterspørselen ikke er vår** (4 588 visn): «bondhus» er en bre, «lista» er stedet, «rennsteig» en tysk turløype, «handi» er hjelpemidler; Crispi/Morakniv/Brusletto/Husqvarna/Snickers ligger i forbrukerkanalen. Fem av dem har fått malen. **Metodefunn:** 🔴 Serper duger ikke — den returnerer 8–10 treff, utelater de samme hver gang, meldte null forhandlere på «pelicase» der nettleseren fant sju, og fant oss ikke på Zarges/Stahlwille/Mitutoyo/Leatherman. Google sperrer etter ~40 raske søk fra samme IP i over ti minutter. Søkestrengen må være identisk med Search Consoles, ellers faller merker ut. De fire siste (Picard, Bosch tilbehør, Scell-it, Fosen Tools Custom) ble målt da sperren slapp etter ca. en time — alle i «for lite volum», ingen konklusjon endret. Rapport: `~/Desktop/FT-analyse-18sept/merker.html`. Memory: `project_merkekonkurranse_maalt`. |
| 19. sept (runde 3) | **Sesong, merkefall og konteksten som endrer alt.** **Julekalenderen haster:** fjorårets sesong ga 8 744 visn og 345 klikk, og 8 733 av visningene lå på produktsider — kampanjesiden `/verktoy-julekalender` er indeksert, men har aldri hatt én visning. Bærerne (Wera-2025 og Milwaukee-**2024**) har falt fra plass 5–6 til 10–11, og **ingen 2026-side finnes**, mens noesen.no er nr. 1 på «milwaukee julekalender 2026» og Elkjøp/Verktygsboden selger 2026-kalendere. Årstallet står i søket: «2025»-søkene ga 336 visn på samme tid i fjor, «2026» gir 11 nå. Sesongen topper uke 45–49. **Julekalender er eneste sesongklynge som betaler** (3,31 % mot 0,52 % hjulskift og 0,00 % hage) og de to kalenderne er **de mest handlekurv-lagte produktene på hele nettstedet**. **Hele årsnedgangen er ett søk:** −707 klikk fordeler seg som eget merke −823, leverandørmerke +287, generisk −179. «fosen tools» gikk fra 888 til 180 klikk på **uendret posisjon 1,0 med flere visninger** — klikkene gikk til Googles bedriftspanel og vår egen annonse. Brand Search ga 1 526 klikk til 1 741 kr; **samlet merketrafikk er opp fra 1 056 til 1 759**. **GA4 juni–august (eneste rene vindu): 11 034 → 10 963 sesjoner, 45 → 45 kjøp, 4,1 per 1 000 begge år.** Organisk gir færre sesjoner men flere kjøp (19 → 24). **Nettbutikken er 3,0 % av selskapet** (726 183 kr, 203 ordrer) og nettstedet har bare 61 telefonklikk og 50 e-postklikk på et år — kontakten skjer i bedriftspanelet. **Det panelet viser 3,4 på ni anmeldelser og «Eier du denne bedriften?»** og er nå tiltak 0. **Et løft på fem plasser er verdt 2,3 klikk per side per kvartal**, likt for alle sidetyper; å falle koster mest på produsentsider. **Packout:** 97,5 % av 3 913 visn ligger på en artikkel på plass 10,8, etterspørselen er åttedoblet, men SERP-en er trang — realistisk ~40 klikk, ikke 150. **Testet og forkastet:** produktbeskrivelser påvirker ikke synlighet (130 sider, flat kurve), og URL-endringer koster ikke rangering (20 produkter, ingen forskjell). **Forkastet underveis:** «hele Direct-tapet er mobil» — 45 kjøp i kvartalet bærer ikke oppdeling på enhet. **Internsøket (14 082 søk) avslører hvilke merker vi eier:** forholdet internsøk/Google-visninger er 28 250 ‰ for Opticase, 752 ‰ for PB Swiss og 308 ‰ for Snap-on, mot 9–17 ‰ for Ledlenser, Zarges, Snickers og Pelicase. Toppen er merkene kundene går direkte til oss for; bunnen er der vi er én av seks. Sammenfaller nesten perfekt med forhandlertellingen — to uavhengige datasett, samme merkeliste. 500 personer søkte «produkter»/«nettbutikk» i søkefeltet (menysak). **Innholdskanalene målt likt (12 mnd):** Google Ads er mest effektiv med 24,3 kr/sesjon (74 % engasjerte, 5,2 sider, 325 sek) og leverer en femtedel av nettomsetningen på en åttendedel av trafikken. **Nyhetsbrevet 19 056 kr på 1 156 sesjoner mot Facebooks 1 372 kr på 1 088** — tretten ganger per besøk. Forskjellen ligger i landingssiden: nyhetsbrevet lenker til `/kampanjer/knipexkupp` (19 % i kurv, best av alt), Facebook til forsiden og `/jubileum`. Regel: lenk til kampanje- eller produktside. (Forbehold: 97 % av salget er utenfor nett, så SoMe kan ikke måles på nettomsetning alene.) **Pmax-nedstengningen var forsvarlig:** 7,7× ser bra ut, men 71 % av sesjonene landet på forsiden og 103 033 av 112 036 kr kom derfra — merkeintensjon, ikke ny etterspørsel. Brand Search gir 12,7×. Om stengingen kostet omsetning kan ikke avgjøres på tre uker; **mål på nytt ~25. okt**. **Referansebanken:** 70 av 200 sider har ≤2 sidevisninger på et år, inngangen er Direct (408 mot 120 organisk = lenker sendt i samtale), engasjementet lavest av alle seksjoner (36 %) — færre og bedre caser. **🔑 Nettstedet har to jobber, og vi måler bare den ene:** forsiden står for 72 av 203 ordrer (7,7 % i kurv), mens B2B-halvdelen (/bransjer, /hdfi, /skreddersøm, /referanser) har 1 910 sesjoner, 0,7 % i kurv og null kjøp — samtidig som /skreddersøm har nettstedets høyeste engasjement (83 %). Det er som det skal være, men med 61 telefonklikk, 50 e-postklikk og 8 generate_lead på et år finnes det ingen måling mellom «leste caset» og «selgeren lukket ordren». **Største enkeltmangel etter tre runder, og ikke et SEO-problem: en målbar kontakthandling på B2B-sidene.** Rapport: `~/Desktop/FT-analyse-18sept/runde3.html`. Memory: `project_seo_runde3`. |
| 19. sept (ytelse) | **Feltdata på ytelse hentet — uten API-nøkkel, og svaret snur saken.** Tjenestekontoen kan ikke lage nøkler (`Permission denied to enable service`), og keyless API-kall gir alltid 429 på den delte kvoten. Men **pagespeed.web.dev sitt eget grensesnitt viser de samme CrUX-dataene** og kan kjøres i nettleseren. **Mobil består, datamaskin stryker:** LCP 0,9/0,7 s, INP 105/39 ms — men **CLS 0,01 på mobil mot 0,3 på datamaskin**, tre ganger over grensen, på den halvdelen av trafikken som kjøper mest. 🔑 **De 791 kB CSS og 850–1 640 kB JS plager ikke ekte besøkende** — Lighthouse sine 45/100 og 20,8 s LCP er en Moto G Power på strupet 4G, mens folk laster siden på 0,7–0,9 s. **Ytelsesprosjektet er avlyst.** **Layout-hoppet reprodusert** med PerformanceObserver: ett utslag ved 660 ms der tekst flytter seg ~6 px opp og blir ~9 px bredere (skriftbytte), vokser med bredden (0,047 ved 1024 px, 0,088 ved 1920), og er nær null med varm cache — det rammer førstegangsbesøkende. Samtykkebanner og header er `position: fixed` og er frikjent. **Den ene render-blokkerende ressursen er Google Fonts med `Playfair+Display:300,400,700|Roboto:400,700`, og alle tre Playfair-vektene står som «unloaded»** — hentes hver sidelasting, brukes ingen steder. Tiltak: fjern Playfair, legg til `&display=optional`. Forbehold: jeg fikk hoppet til 0,088, ikke 0,3 (75-persentilen hos ekte brukere), så Multicase bør bekrefte. **Nøkkel lagt inn samme dag og full måling kjørt** (`scripts/pagespeed-feltdata.mjs`, 7 sider × mobil/desktop, ~7 min — kjør i bakgrunn). To funn til: **CrUX har ikke data per URL** for fosen-tools.no (alle sider svarer `origin_fallback`, ett tall for hele domenet), og **CrUX rapporterer CLS som heltall ×100** (30 = 0,30) — begge feilene lå i første scriptversjon og er rettet. Labdata datamaskin: produktside 91, kampanjeside 77, forsiden 66, kategoriside 64, referansecase 61, produktkatalogen 50 og **produsentside 44 med 5,79 s LCP — tregest av alle, og det er sidetypen som konverterer best på merkesøk.** 502–605 kB ubrukt JS og 194–200 kB ubrukt CSS på hver side (felles rammeverk). Mobil-labtallene er tidsavbrudd, ikke målinger (LCP 21–253 s) — bruk feltdataene der. **Tjenestekontoen slipper forøvrig inn på PageSpeed uten nøkkel med scope `openid`**; cloud-platform og webmasters.readonly gir 403. Oppskrift og resultat: `~/Desktop/FT-pagespeed-nokkel.html`. Memory: `project_ytelse_malt_i_nettleser`. |
| 19. sept (sidevekt) | **Forsiden veier 15,4 MB, og 7,6 av dem er vår egen logo-vegg.** Målt avduplisert på adresse i ekte nettleser. Det ligger to logo-komponenter på forsiden: Multicase sin egen karusell henter 4 logoer gjennom plattformens bildeskalerer (`_scaled_320.Svg`, **16 kB i snitt**), mens **vår `ft-logo`-vegg henter 55 originalfiler rått** fra `/userfiles/image/Logo/` — **138 kB i snitt, åtte ganger tyngre**. 25 av logoene er ekte vektor og veier 350 kB til sammen (median 10 kB); **30 er PNG pakket i SVG og veier 7 229 kB = 95 % av vekten**. `The-Bone.svg` er 3 260 kB, nettstedets største enkeltfil. Å tegne om de 30 henter inn **≈ 6,9 MB = 45 % av sidevekten**. **Ærlig om verdien:** ekte besøkende merker det ikke (CrUX 0,65–0,92 s), så det er ikke en rangeringssak — men det er 6,9 MB per besøk på mobildata, en sannsynlig bidragsyter til desktop-CLS på 0,30, og en formiddags arbeid. **🔴 Fire påstander fra første versjon var feil** (110 forespørsler, 20,7 MB, dobbelthentede videoer, «tre separate Google-tagger» — det siste er GTM + GA4 + Ads, riktig oppsett): **Resource Timing fører opp én rad per delforespørsel på mediefiler**, og jeg leste rader som forespørsler. Avdupliser alltid på `entry.name`. **Ny feil funnet samtidig: merkeveggen på `/produsent` er tom på mobil** — `.ProductMenu` finnes ikke i DOM under 768 px, så veggen får 0 kort på en side som lover «mer enn 150 leverandører». Alvorlighetsgrad lav: 196 visninger og **0 klikk** på 90 dager, 102 sidevisninger i GA4, 21 s på siden på mobil mot 49 s på datamaskin. Rapport: `~/Desktop/FT-logoer-og-sidevekt.html`. Memory: `project_sidevekt_logoer`, `feature_merkevegg_produsent`. |
| 19. sept (julekalender) | **Julekalenderen 2026 haster, og fristen er målt.** Fjorårets sesong okt–des: **13 152 visninger, 499 klikk, 3,8 % klikkrate** — 3× nettstedets normale (tallet ble rettet opp samme dag, se sesong-raden), og eneste sesongklynge der vi både rangerer i topp 5 og blir klikket på. 🔑 **Wera-2025-siden ble lagt ut ~1. juni 2025 og brukte åtte til ti uker på å klatre** fra posisjon 48,9 (første visning 18. juni) til 8,8 i midten av juli og 5,0 i sesongen. En side som legges ut i dag er ensifret rundt midten av november. **41 % av sesongens visninger kommer før 1. november** (klikkene topper i uke 48–49 med 73 og 71), så oppbyggingen er i gang nå — vi ligger på 480 visninger 20. aug–16. sept, omtrent som i fjor. **Milwaukee er det største søket** (2 444 visn, 137 klikk, pos 5,3), og eneste Milwaukee-side er **2024-utgaven**. Årstallet i søket er et kjøpssignal: «wera julekalender 2024» ga 17,6 % klikkrate. **Posisjonene faller allerede** mot i fjor: milwaukee 5,3 → 9,3, wera 3,5 → 8,2. Sitemap gjennomgått: fire julekalender-adresser, **ingen for 2026**. Mønsteret som virker er **egen produktside per årgang** (`/wera/123481/verktøy-julekalender-fra-wera-2025-adventskalender`), og de gamle skal ikke slettes — Wera-2024 har 2 292 visn / 129 klikk og fanger fjorårets årstall. **Kampanjesiden `/verktoy-julekalender` er indeksert og crawlet 18. sept, men har aldri hatt én visning på to år** — produktsidene tar søkene selv (pos 3,3), så den trenger ikke fikses. Det som blokkerer er innkjøpsbeslutningen; konkurrentene er i gang (noesen.no er nr. 1 på «milwaukee julekalender 2026»). Rapport: `~/Desktop/FT-analyse-18sept/julekalender.html`. Memory: `project_julekalender_sesongklyngen`. |
| 19. sept (sesong) | **Systematisk sesongsøk over alle 691 søkeord med 150+ visninger** — julekalenderen var funnet ved å gjette tre klynger, dette er hele bildet. Metode: 12 hele måneder, 20 950 søkeord, sesongbetont = ≥45 % av visningene i ett kvartal. 🔴 **Første rettelse: julekalender-klyngen er 60 % større enn jeg målte samme morgen** — filteret på ordet «julekalender» hoppet over «adventskalender», «verktøy kalender» og «milwaukee kalender» (4 956 visn / 154 klikk). Riktig er **13 152 visn / 499 klikk**, og 2026-titlene må inneholde begge ordene. **Lærdom: aldri mål en klynge på ett ord.** **To funn som er aktuelle nå:** `verktøyvogn uten verktøy` gir 1 260 visn og 49 klikk med **3,9 % klikkrate på plass 6,5**, 81 % i sep–nov og **null** jan–aug — og den svares av `/produkter/verktøyvogner`, **samme side som falt fra plass 10 til 59 på generisk «verktøyvogn»**. Siden er altså ikke død, den har bare mistet hodeordet, og bør stå i fred til desember. `luftblåser` gir 1 871 visn og 27 klikk på plass 7,6 med topp okt–nov, men svares av **én produktside** (plass 10,8) mens kategorisidene ligger på plass 28 og 45 — ingen samleside tar ordet, og det er den billigste muligheten. **Hjulskift-ombyggingen 27. aug er for tidlig å kalle:** samme vindu ga 458 visn / plass 21,8 i fjor mot **42 visn / plass 9,7** i år — tolv plasser bedre, synlighet ned 90 %. Oktober er toppmåneden, så mål ~10. okt. Året sett under ett har `hjulskift` 2 277 visn og **1 klikk** på plass 31, som korrigerer «hjulskift er lite og fallende» — etterspørselen er der, posisjonen mangler. Resten av året: des–feb tang verktøy 2 851/7 og grisepikk (topper i februar), mar–mai pipesett 4 008/10, jun–aug packout 5 584/19 og rissenål 2 098/7. ⚠️ `tools` har 46 236 visn i sep–nov men 14 klikk på året — merkeforvirring, holdt utenfor. Rapport: `~/Desktop/FT-analyse-18sept/sesong.html`. Memory: `project_sesongsok_systematisk`. |
| 19. sept (sidetype) | **Når feil sidetype svarer — 69 generiske søk, 44 047 visninger, 343 klikk.** Målte alle søkeord–side-par på 12 mnd (merkenavn utelatt, ≥250 visn) og fant hvilken side som får flest visninger per søk. Der svaret er **én enkelt produktside** på et generisk flertallsord, er det sannsynligvis feil sidetype. To varianter: **ingen kategoriside finnes** (luftblåser 1 698 visn, nålebanker 1 501, oppretterverktøy 1 258, universalledd 692) eller **den finnes og Google velger den bort** (billøftere prod 9,1 / **kat 70,2**, gardintrapp 4,5/49, industristol 15,3/30,6, arbeidsplattform 11,8/23,6). 🔴 **Motprøven var feil, rettet 20. sept:** «jegerbar, kategorisiden på plass 2» bygget på **én visning**. Se sidetype-skala-raden. **Ikke rør blindmuttertang** (3,8/6,9, begge side 1, gir klikk). **Fallsikring er det største tilfellet og en uttalt satsing:** etterspørselen er **åttedoblet** på et år (2 651 → **21 994 visninger**, 22 → 83 søkeord), posisjonen tolv plasser bedre (61,6 → 49,3), og vi fikk **seks klikk**. 🔴 **Hypotese testet og forkastet før publisering:** jeg trodde klyngen i hovedsak var kunnskapssøk (dimensjonering, krav, årskontroll) og at en fagartikkel var veien inn — **kunnskapssøk er bare 11,5 % (2 524 av 21 994)**. Det er posisjonsmangel, ikke innholdsmangel. **Hodeordene er utenfor rekkevidde** (`fallsikring` 3 896 visn på plass **67,1**, `fallsikringsutstyr` 2 638 på 67,2), **bransjeordene er nærmest** (`offentlige bygg` 819 på **18,1**, `industri` 763 på **20,7**) — angrip dem, ikke hodeordet. Fem sidetyper konkurrerer om de samme ordene, og landingssiden **`/fallsikring` har én visning på plass 100**. ⚠️ Avklar først om FT tilbyr årskontroll som tjeneste — søkene finnes, men selesertifiseringen står uavklart, og det avgjør om dette er produkt- eller tjenestesatsing. Rapport: `~/Desktop/FT-analyse-18sept/sidetype.html`. Memory: `project_feil_sidetype_svarer`. |
| 20. sept | **Kategorisidene målt i skala — og gårsdagens motprøve var feil.** Jeg hevdet at Google ikke nedprioriterer kategorisidene våre, med `jegerbar` som bevis. Den «plass 2» bygget på **én visning**; jegerbar er tvert imot samme problem (616 visn på åtte produktsider rundt plass 7, null klikk). Og «ingen kategoriside finnes» stemte bare for 2 av 4 — oppretterverktøy har to, universalledd har tre. **Rotårsak: jeg leste søkeord–side-par innenfor en rowLimit, og sjeldne par faller ut. Sjekk alltid med `operator: equals` før du påstår at en side ikke finnes.** **Testet ordentlig på 291 generiske søk** (≥250 visn, 12 mnd, merkenavn filtrert): produktside vinner 146 søk med 0,56 % CTR på snittplass 13,5, **kategoriside vinner 145 med 0,33 % på snittplass 28,4** — like mange søk, femten plasser lenger bak, halve klikkraten. **Produsentside 2,94 %, sjette uavhengige bekreftelse.** Direkte: på 69 søk der begge rangerer står produktsiden foran i **47 (68 %)**. 🔴 **«Bredde avgjør» ble testet samme dag og forkastet.** Jeg tolket mønsteret som at utvalg avgjør og foreslo produktantall som port. Talte produktene på ti kategorisider i nettleser: **kjørner har 11 produkter og ligger på plass 11, containere har 42 og ligger på 30,6**; bladføler 14 på plass 12,4, borhammer 16 på plass 45,2. Områdene overlapper helt — **verken produktantall eller ordantall forutsier posisjon**. Det som faktisk skiller er at vinnerne er navn på en håndverktøytype og taperne er utstyrs- og maskinkategorier med nasjonal konkurranse. Samme svar som SERP-målingene: **konkurransen på ordet avgjør, ikke siden vår.** Port: bruk **posisjon**, ikke produktantall — plass 9–15 lønner ombygging, plass 30–45 gjør det ikke. 🐛 **`/produkter/containere` har null H1** (overskriften ligger som H2). Rapport: `~/Desktop/FT-analyse-18sept/bredde-testet.html`. **Containere er største enkeltsak:** 23 156 visn, 153 klikk, 0,7 % på 100 søkeord, og **`/produkter/containere` kommer ikke på topp 14** blant sidene som svarer — bare undersidene (plass 25–37). Tre produktsider bærer klyngen (10-fot 7 461 visn @ 31,5, 40-fot 6 861 @ 27,1, 6-fot 3 482 @ 20,6). Motsier ikke at feltet er bemannet av containex/spacebox/finn.no, men legger til at vi heller ikke stiller med riktig side. **Selvnøstende adresser splitter autoriteten:** `universalledd` har kategorisidene sine på `/1-/universalledd` (60,4), `/1/2-/universalledd` (73,0) og `/1/4-/leddpipe` (85,5) — rydd adressene før de får tekst. Rapport: `~/Desktop/FT-analyse-18sept/sidetype-skala.html`. Memory: `project_kategorisider_skala`. |
| 20. sept (kategoriverdi) | **Kontrollert måling av om kategoriombyggingen virker — og hvilke sider den lønner seg på.** Fallsikring/Innredning/Containere ble bygget om i august; resten er kontrollgruppe. Samme to vinduer (1. juni–15. juli mot 1.–16. sept): **de ombygde flyttet 3,8 plasser i median med 15 opp og 4 ned**, kontrollgruppa 0,2 plasser med 43 opp og 39 ned — altså et myntkast. **Posisjonseffekten er reell.** ⚠️ **Klikkeffekten er ikke påvist:** CTR steg i begge grupper (0,53→0,62 % ombygd mot 0,67→0,81 % kontroll), og kontrollen steg mest. Mål på nytt ~10. okt. **Klikkratekurven målt på alle 468 kategorisider:** plass 6–10 gir 1,52 %, 11–15 gir 0,84 %, 16–20 gir 0,69 %, 21–30 gir 0,65 %, 31–50 gir 0,45 %. **Kurven er flat fra 16 til 30** — den bratte kanten ligger mellom plass 15 og 10. 🔑 **Derfor lønner fire plasser seg bare ett sted.** Lagt inn i kurven: **11–15-båndet (79 sider, 26 622 visn) går fra 197 til 318 klikk, +121 — tre fjerdedeler av hele gevinsten**. **21–30-båndet har flest visninger av alle (33 265) og gir ±0**, fordi 25 → 21 er samme bånd. 31–50 gir +3. Samlet for alt over plass 10: **~160 klikk per 90 dager** — som konvergerer med det uavhengige anslaget på ~175 fra 7. sept, beregnet på helt andre tall. **Anbefaling: ta 11–15-båndet først, ikke de dårligste sidene** (29 sider med ≥250 visn: skuffer, rørtang, skrallenøkler, fastnøkler, bukker, hyller, avbitertang, kjørner, kulehammer, drivdor …). Ikke bruk tid på 21–50 med mindre det er andre grunner enn søk. Containere er unntaket — fikses fordi H1 mangler. Rapport: `~/Desktop/FT-analyse-18sept/kategoriarbeidet.html`. Memory: `project_kategoriarbeidet_verdi`. |
| 20. sept (malene) | **Begge malene kjøper posisjon, ingen har levert klikk — og «produsentsider leverer 3 %» er halvparten Snap-on.** To før/etter-målinger med kontrollgruppe: **kategorimalen** (aug) flyttet 21 sider **+3,8 plasser i median, 15 opp / 4 ned** mot kontrollens +0,2 og 43/39; **produsentmalen** (29. apr) flyttet 15 sider **+5,5 plasser, 9 opp / 2 ned** mot kontrollens +0,5 og 13/11 (Viking Arm 52,7→19,1, Fluke 31,9→14,6, Bahco 32,0→17,0, Milwaukee 28,6→15,4; Lista AG og Gigant gikk bakover). ⚠️ **Klikkeffekten lar seg ikke skille fra støy i noen av dem** — kategorimalen steg mindre enn kontrollen, produsentmalen falt. 🔴 **Korreksjon av et tall som har stått seks ganger:** alle 153 produsentsider gir 3,24 %, men **`/snapon` alene er 799 av 1 537 klikk = 52 %**. Uten Snap-on: **1,73 %**. Gårsdagens 2,94 % fra generiske søk er ikke Snap-on (2,98 % uten), men **tre sider bærer 91 % av klikkene og to av dem er ikke produsentsider** (`/outlet` 766 visn/95 klikk kom med fordi adressen har ett ledd, pluss `/gaveartikler`); fem av tolv søk ga null klikk. **Ryddet: 2,09 %.** **Kontrollert for posisjon (plass 6–15) finnes ingen målbar CTR-fordel av malen:** med mal 1,72 % samlet / 0,97 % median per side, uten mal 3,48 % / 1,22 %; uten hver gruppes største side 1,54 % mot 1,81 %. Gruppa uten mal er liten og spredt, så malen skader neppe — men fordelen finnes ikke i tallene. **Produsentsider konverterer bedre fordi de rangerer på merkesøk, ikke fordi de har malen.** **Sidefunn: topp 5 er vår dårligste plassering** — produsentsider uten Snap-on gir 0,54 % på plass 1–5 mot 2,20 % på 6–10, fordi `/fosen-tools` (1 459 visn, 5 klikk) og `/kontaktoss` (1 396 visn, 7 klikk) står der på eget navn og bedriftspanelet tar klikket. **Følge: malene er rangeringsverktøy, ikke konverteringsverktøy. Ikke bygg flere produsentsider på CTR-argumentet.** Rapport: `~/Desktop/FT-analyse-18sept/malene.html`. Memory: `project_malene_posisjon_ikke_klikk`. |
| 20. sept (kanalene) | **`campaign_traffic` analysert for første gang — og høsten 2025 var 44–60 % bot.** Kanalen «Unassigned» er tredje størst siste 12 mnd (7 547 sesjoner) med **0,7 % nøkkelhendelser og 3,8 % engasjement**, og nesten alt kom i én bølge: **sept 2025 var 60 % Unassigned (6 023 av 10 043), okt 46 %, nov 44 %, des 0,8 %**. Signatur: 6 552 sesjoner fra land «(not set)» på desktop med 0 % engasjement, Singapore 254, Kina 142, og 4 804 uten landingsside. **Reell høsttrafikk var ~4 000/mnd, ikke 7 000–10 000.** 🔴 **Dette vil gi et falskt fall på 40–60 % når okt/nov 2026 sammenlignes mot i fjor** — trekk fra Unassigned først. Det forklarer også «høsttoppen 2025» som sto uforklart: den var aldri en topp. **Innholdsmaskinen målt:** 6 835 kampanjesesjoner feb–sept fordelt på Google Ads 4 539 (66 %, 19,7 % hendelser), nyhetsbrev 1 303 (19 %, 10,5 %), **alt sosialt 565 på 71 kampanjer (8,3 %, bare 1,9 %)** og ChatGPT 217 (19,8 %). Ukentlige poster enkeltvis gir **7–18 sesjoner hver**; LinkedIn 15 på sju kampanjer, Instagram-bio 31 på fem, jubileet 141. ⚠️ Måler bare klikk på merkede lenker — 97 % av salget er utenfor nett og IG har ingen lenke per innlegg, så ikke kutt i sosialt på dette alene. 🔑 **AI-trafikk er den best konverterende kanalen vi har:** 456 sesjoner og 99 nøkkelhendelser = **21,7 %**, mot organisk søk 13,2 %, e-post 10,5 % og organisk sosialt 3,2 %. **Underrapportert i GA4** — kanalen «AI Assistant» viser 217, men 142 ligger under Unassigned. ChatGPT alene gir halvparten så mange sesjoner som alt sosialt og **3,3× hendelsene**, uten at noen har gjort noe for det. Fortjener egen linje i ukesrapporten. **Opprydding:** 42 av 132 kampanjenavn er Mailchimps autogenererte `…-EMAIL_CAMPAIGN_…`. Rapport: `~/Desktop/FT-analyse-18sept/kanalene.html`. Memory: `project_kanalene_og_botflommen`. |
| 20. sept (gjenstående) | **Dekningsgjennomgang av alle 16 rapporter og 50 minnefiler for å finne hull i stedet for å grave i det samme — to fantes.** 🔴 **FT Aviation forsvant fra Google i seks uker, 20. juli–30. august.** Juni ga 15,6 visn/dag på 93 sider, juli 4,7, **august 0,84 på 39 sider og null klikk** — hele augusts trafikk (26 visninger) kom på én dag. 70 av 93 juni-sider var borte. **Nettstedet var ikke nede:** GA4 viser sammenhengende trafikk (12/5/27/31/35/21 sesjoner per uke), og URL Inspection viser alle sider indeksert med `SUCCESSFUL` henting. **Årsaken er ukjent.** ⚠️ **Min første lesning var feil** — jeg regnet 3,1 → 19,8 visn/dag som «seks ganger bedre», men «før» var et snitt over sammenbruddet: **september er tilbakevending til juni-nivå, ikke gjennombrudd.** Og oppgangen begynte **31. august, dagen før SEO-jobben** (27 → 67 → 62 → 100 den 2. sept da sitemapen ble meldt inn), så arbeidet kan ikke krediteres gjenreisningen. **✅ Varsling bygget samme dag** i `scripts/fta-ukesrapport.mjs`: boks øverst i rapporten med fire nivåer, basert på GSC-visninger alene siden GA4 ikke ga noen advarsel. Kritisk ved ≥5 dager uten visninger eller <50 på sju dager mot et snitt på ≥50; advarsel ved fall ≥70 %. Tørrkjørt mot ekte data: **ville slått ut 25. juli, fem dager etter at sammenbruddet startet**. **Nyhetsbrevet, andre største målbare kanal, aldri gjennomgått:** åpningsraten har steget jevnt i seks år fra 27,07 % (2020) til **35,29 % (2026)** — over B2B-snittet — mens **klikkraten er halvert mot 2020 (6,07 → 3,19 %) og en fjerdedel av 2023 (14,25 %)**. Folk åpner og lar være å klikke; arbeidet bør ligge i innholdet, ikke emnelinjen. **Listen krymper:** uten jubileums-importen på 665 i februar er den ned 181 siden mars (66 påmeldte mot 186 avmeldte), og popupen ble tatt ned 17. aug, så det finnes ingen mekanisme som henter inn nye. 🐛 **`mailchimp_list_growth` virker ikke** — null i alle felter unntatt `cleaned` i samtlige 65 rader; bruk `mailchimp_list_daily`. ⚠️ **To ganger denne dagen var jeg nær å publisere feil fra å anta hva et felt betyr** (Mailchimp-åpningsrate regnet til 184–262 % fordi `reach` er sendt og `likes` er unike åpninger — API-et regner riktig). Sammen med `jegerbar` og «bredde avgjør» er det fire på to dager, og fellesnevneren er at et tall blir farlig når det bekrefter noe jeg allerede trodde. Rapport: `~/Desktop/FT-analyse-18sept/gjenstaaende.html`. Memory: `project_fta_borte_seks_uker`, `project_nyhetsbrev_helse`. |
| 19. sept (caps) | **Overleveringspakke for capsbygging i Blender.** `~/Desktop/Caps-i-Blender.zip` (616 kB) samler metoden fra 16.–17. sept så den kan gjenbrukes på et annet merke: `LES-MEG.html` med bilder og kopiknapper, `OPPSKRIFT-FOR-CLAUDE.md` for mottakerens assistent, alle 13 script, eksempel-SVG-er og `FILKART.txt` som peker på originalene på T7. Kjernen i metoden: fest aldri mot capsmodellen (318k verts med ekte sekskantbulker) — tilpass en glatt flate og fest mot den; frys bezier-håndtakene til FREE før skalering, ellers smelter bokstavene; del opp geometrien før kurvingen, ikke etter; `bevel_depth` alltid 0 på bokstaver. Samme to-README-mønster som FTA Picturebook-pakka.
...
Fire dager som tok FT Aviation fra null tilstedeværelse til full lansering, pluss picturebook-ombygging.
...
### Picturebook-pipelinen
Originalen (Docs-eksport, indigo/turkis/grå) gjenoppbygd i ekte FTA-stil: 30 A4-sider, alle 62 produkter med originaltekst, originale «More info»-URLer (ekstrahert med pikepdf), dekorbilder med FINAF/Lockheed-copyright, C-130-merker. PDF er master; PPTX er kopi (aldri 1:1-jakt — avblåst). **Overlevering:** `FTA-Picturebook-pakke/` med selvstendig generator + LES-MEG for pappa + OPPSKRIFT-FOR-CLAUDE.md for hans Claude. Mønster å gjenbruke: README for mennesket + README for mottakerens Claude.

### Lærdommer
- pptxgenjs: sizing contain/cover kan strekke — regn contain selv + pre-crop fullflater; interne slide-lenker skriver korrupt fil.
- Chromium-print: gradient-border-triks og box-shadow blir firkanter — enkel CSS i PDF-pipelines.
- ft-aviation.no stripper UTM ved 301 fra kortformer — bruk alltid full slug-URL.
- Klikkbare YouTube-lenker krever ID-verifisering (telefonverifisering gir kun thumbnails).
- Tom Multicase-container i HTML = admin-config, ikke CSS (attributt-webikoner: manglet, fikset av Adrian i admin).

### Fredagsposter klare
22/8: VDE-innlegg (verktøykontroll-vinkling, artikkelnummer-merking). 29/8: gaveartikkel jerrykanne-koffert (Dråben Montasje AS → Bratsberg Gård; alkohol ikke nevnt, etiketter fjernet med generativ fyll). Begge med UTM-CTA til kontaktsiden (`vde-innlegg-2026-08` / `gaveartikkel-2026-08`). Filer på skrivebordet.

---

## Siste sesjons-sammendrag (17. august 2026 — mandagsøkt)

Tre spor på rekke: møtedata, nyhetsbrev, referanser.

### Ukesrapport uke 33
GA4 sto stille (913 mot 915 sesjoner). Det som beveget seg: **leads doblet fra 7 til 14 uten at kostnaden endret seg** (311 kr begge uker, kost per lead halvert til 22 kr), og **nyhetsbrevet 11. august fikk 1,8 % klikkrate** mot 0,3–0,4 % på de fire foregående. Meta falt til støynivå (engasjement 13 mot 39, beste innlegg nådde 51). SEO +10 % klikk, nesten alt fra Snap-on som nå står på posisjon 1,0 for «snap on norge». ROAS «falt» fra 15,25x til 3,70x, men med ett kjøp i uka er tallet ren tilfeldighet og ble holdt utenfor briefen. Milwaukee-utsendelsen 4. august fikk 0 klikk på 605 åpninger — undersøkt og **ikke en kodefeil**: sporing, lenker, bilder og CTA-er var i orden i alle tre utsendelsene som ble sammenlignet.

### Nyhetsbrev 18. august
Tema valgt på data, ikke smak: pipesett-utgaven vant med formatet «én kategori, flere merker», og søkeklyngen sekskant/torx/bits har **1030 visninger mot 18 klikk** siste fire uker, altså en stor udekket etterspørsel der vi rangerer på plass 7–11.

Lærdommer verdt å ta med:
- **Lager må sjekkes per lokasjon, ikke bare `in_stock`.** `.main-warehouse .DynamicStockTooltipContainer` gir antall og avdelingsnavn. Snap-on røk fordi eneste eksemplar sto på Sør.
- **JSON-LD-bildet kan tilhøre et søskenprodukt.** Wera Torx-settet har ikke eget bilde, og scraperen returnerte unbrako-settets foto. Sjekk at bildene faktisk er distinkte.
- **Multicase serverer WebP uansett `.jpg`-endelse.** Uten konvertering til JPG på hvit bunn blir de svarte i Apple Mail.
- **Malen arver en utdatert GA-kampanjekode** (`EMAIL_CAMPAIGN_2026_02_...`). Den legger på egne `utm_*` som kolliderer med våre. Slått av på denne utgaven.

### Referanser
Fire nye 2025-caser flyttet fra «Ikke lagt inn ennå» til «Allerede ute». Årsfilter lagt på de fem kategorisidene som manglet det, med samme `a:fra:til`-mekanisme som vogner allerede brukte — alle chips testet ved å klikke dem i nettleser og telle synlige celler.

To feil funnet: containere-rutenettet pekte på `container-workshop-lista` som ikke finnes (rettet til `container-lista`), og luftfart-caset var flyttet til verkstedinnredning **under nytt navn** `hyllesystem-til-luftfart`. Landingsskriptet forkastet stille de fire nye casene fordi meny-fasiten i `scripts/_tmp-*-meny.json` var eldre enn dem — oppdatert, og landingssiden bygget med **177 caser**.

### Feil jeg gjorde underveis
- Kalte filtrene på vogner og kofferter «i stykker» etter å ha lest et avkortet script-utdrag. De virker; jeg hadde bare ikke sett `a:`-grenen. Live-test av hver chip avkreftet påstanden.
- Renummererte JSON-LD-posisjoner på tvers av alle schema i en blokk, så BreadcrumbList og ItemList ble én sekvens. Fanget i verifisering og rettet til per-schema.
- Fjernet «Ikke lagt inn ennå»-banneret uten å flytte seksjonene, så casene ble liggende uten overskrift. Rullet tilbake fra backup og gjorde det på nytt.
- Pushet til Mailchimp fire ganger før Adrian sa at det ikke skal gjøres.

### Ettermiddagen — datarevisjon («er tallene vi viser riktige?»)

Adrian ba om full gjennomgang av analysegrunnlaget. Fire feil funnet, alle med samme lærdom: **API-er fjerner metrikker stille, og koden svelger feilene stille.**

1. **Meta side-insights** — `page_impressions_unique` fjernet i Graph v22; hele kallet feilet, rekkevidde sto på 0. Fikset (`fe9b613`).
2. **IG-media-insights** — `impressions`/`plays` fjernet, erstattet av `views`; alle IG-innlegg sto med reach 0. Fikset + backfillet, historikken reparert siden IG-insights er kumulative (`fcf06cd`).
3. **Anomali-varsling** — tre feil ga 34 støyvarsler på 30 dager: manglende paginering (Supabase kutter ved 1000 rader → «tidligere sett» avkortet → samme term «ny» hver dag), pmax-snapshot-summering (rullende 90d-aggregat telt per dag), og stopp-terskel på 3 konverteringer (P(0)≈30 % ved ren tilfeldighet — hevet til 8). Fikset (`9b0d355`), tørrkjørt mot prod: 21 støy → 1 reelt signal.
4. **Kontaktskjema-sporingen** — den alvorligste. Scriptet tolket «iframe lastet ≥2 ganger» som innsending. 29 «leads» 29. juli, null tickets i Freshdesk samme dag (Adrian sjekket — det som gjorde diagnosen til fakta). Phantom-reloads ga falske leads OG kunne bytte ut skjemaet med falsk takke-boks for ekte besøkende. Nytt script med interaksjonskrav (blur + 300ms-polling på `document.activeElement === iframe`) publisert og verifisert live. Gjenstår: «én per klikk»-telling i Google Ads.

**Konsekvens for uke 33-briefen:** «leads doblet 7→14» var ett bot-burst 13. august; reelt nivå er 1–3/uke. Meta var heller ikke så død som vist — IG-innleggene nådde 56–199 personer, tallene manglet bare i databasen.

**Friskt:** GA4 uten hull/duplikater, Google Ads-dekning komplett, `conversions` ≤ `all_conversions` overalt, `begin_checkout` holdt utenfor leads i alle fire API-er, Mailchimps 13 rader/90d er by design (én per utsendelse).

**Bonusfunn:** Keyword Planner Basic Access er godkjent av Google (søkt april) — tjenesten slo seg på selv via graceful degradation. `/innsikt/vekst` og rapport-arkene leverer nå ekte volumtall.

**Annet samme dag:** Meta-token → permanent System User (utløper aldri; side-token utledes i koden), nyhetsbrev-popup avpublisert etter tall-beslutning (0,21 % opt-in), FTA-videomappene omorganisert per produkt (render-script følger), NCAGE N6940 inn i FT Aviation-footeren, GTM-pause for popup-taggen ligger upublisert i Adrians arbeidsområde.

### Kvelden — kontaktskjema lukket + Google Ads ryddet
- **Kontaktskjema-fiksen publisert og bevist.** Adrian sjekket Freshdesk: null tickets 29. juli mot 29 «leads» — phantom-reloads bekreftet, ikke skjema-spam. Nytt script (interaksjonskrav via blur + polling, styrer både event og takke-boks) limt inn i Multicase, verifisert med 7 markør-sjekker og en ekte testhenvendelse i GA4 Realtime.
- **Google Ads-konverteringer ryddet:** form_submit-telling «Alle»→«Én konvertering»; død AW-tag «Kontaktoss skjema» (0 konv., «Handling kreves») fjernet — bekreftelsesdialogen garanterte null kampanjepåvirkning. Oppdaget at rollene var snudd fra april-doc: **GA4 form_submit er primær**, ikke AW-taggen. Google har altså budgitt mot bot-forurenset signal i ukevis; renset fra i dag.
- **Nesten-uhell verdt å huske:** «Endre målet»-dialogen pre-stager promoteringer til primær («1 er endret» i forhåndsvisningen) — avbrutt før lagring. Feilkonfigurert-merkene på handlekurv/betalingsprosess/sidevisning er by design (sekundær-only) og skal IKKE «fikses».
- **Kofferter-filteret** byttet fra størrelse til årstall etter Adrians beslutning — alle 66 dekket (32 var udekket pga. mellom/stor-vs-medium/store-splitten).
- **Popup:** exit-intent-versjonen ble satt opp, men Adrian besluttet avpublisering — 0,21 % opt-in på 7 461 visninger. Skjemaet ligger arkivert i Mailchimp om det skal gjenopplives.

### Åpent
- **Fire tomme kundefelt** (TESS, Salmonor, Økotech, Pro Support) står fortsatt. Merk at tomt kundefelt ellers er med vilje — se `feedback_aldri_gjett_kundenavn`.
- Skanning av faktabokser må bruke `networkidle` og vente på `dl.ft-case__fakta`; den lastes via AJAX og `domcontentloaded` er for tidlig.

## Siste sesjons-sammendrag (11.–12. august 2026 — referansebanken)

To lange dager på `/referanser`. Første dag ble strukturen ferdigstilt, andre dag ble den fylt.

### Ferdigstilling og opprydding
`/referanser` sto ferdig med 73 caser, 8 kategorier og landingsside. Alle lenker ble verifisert mot **admin-ProductMenu**, som er den eneste pålitelige kilden: Multicase svarer aldri 404, så en gal lenke lander stille på forsiden. Fire døde lenker i våpenlagring ble funnet på den måten.

**Redirects ble droppet etter regnestykke.** Lista på 65 rader var bygget, men de gamle adressene hadde til sammen **14 klikk og 1101 visninger på 90 dager**, og hver adresse har trolig 60+ land-prefiks-rader i redirect-modulen. `/referansergammel` er slettet. De gamle adressene går nå i en firehopps-kjede som ender på den robots-blokkerte søkesiden.

### Ny kategori: verktøyinnlegg
De åtte eksisterende kategoriene er alle definert av **beholderen**. Den niende er definert av at FT bare har levert innlegget. Ingen av de 73 eksisterende casene hørte hjemme der.

Slug-valget ble tatt på GSC-data: «skuminnlegg» har mest volum (~290 visn./180d) men eies av DIY-produktsidene som selger, og «hdfi» er allerede splittet på åtte URL-er. «Verktøyinnlegg» har lite volum, men ingen eier det.

### 14 nye caser
Bygget fra bildearkivet, ett prosjekt om gangen. Kategorifordeling: 4 vogner, 4 kofferter, 5 innlegg, 1 verkstedinnredning.

### Verktøyet
`scripts/ref-bilder.mjs` tar en mappe og gjør den klar for opplasting: EXIF-rotasjon, skalering til 1600 px, `IMG1.jpg` og oppover, utmappe navngitt som Multicase-stien, og ferdig `bilder`-linje til datafila. `--rekke` styrer slider-rekkefølgen, `--kun` begrenser til de valgte. Typisk komprimering: 60 MB inn, 2 MB ut.

### Faktaboks-bugen
Kunde, Bransje, Løsning og Levert forsvant på flere sider. Årsaken var **ikke** innlimingsmodus, slik jeg først antok. Markupen brukte `<div>` inne i `<dl>`, som er ugyldig etter HTML4, og Multicase strippet dem. Byttet til `<div>` og `<span>` i alle generatorene. SCSS-en støtter begge strukturer, så sider som allerede sto ute forble riktige.

### Regler som ble låst
- **Maskinert, ikke gravert.** Gravert er lasergravering på verktøyet. Maskinert er når HDFI lages. «Skummet» skal heller ikke brukes.
- **Aldri gjett tall, navn eller beholdertype.** Skal noe stå i teksten, må det være verifisert. Fire bommer på én dag: «to vogner» der det var én, «koffert i to lag» der det ikke fantes koffert, «tjue skuffer» der det er ti, og «Kongsberg Aviation Maintenance Services» der bare «Kongsberg Aviation» var lesbart før bildekanten kuttet teksten. Fellesnevneren er å komplettere noe man ser halvparten av.
- **Fredagsposter inviterer til forespørsel**, ikke bart «Lenke i bio», og lenken går til kontaktsiden.
- **Stempelet leses «Tool control by Fosen Tools»**, med HDFI-logoen i midten.

### Kildearkiv og bildebehandling
Nye caser bygges fra `/Volumes/01-Fosen Tools/Bilder/{år}/{Kunde}/`. RAW-filer konverteres med `sips` først.

Rot i verkstedbilder ligger langs kantene, reoler og tak øverst, gulv og paller nederst. `scripts/ref-blur.mjs --topp 0.30` eller `--bunn 0.16` legger en gradient fra kanten som ikke kan svikte langs konturen av motivet, slik en utklippsmaske ville gjort. Samme mulighet finnes som `--blur` i `ref-bilder.mjs`. Adrian frilegger selv i Photoshop der bildet fortjener det, og de filene mates rett inn i pipelinen.

**Åpen ved sesjonsslutt 12. august, ikke opprettet i admin ennå:**
- Verktøyvogner: `verktøysett-til-hystar`, `verktøyvogn-til-helikoptervedlikehold`
- Verktøyinnlegg: `innleggssett-til-luftfart`, `innlegg-til-verktøyskuff`, `innlegg-til-trykkluftverktøy`

Alle andre kategorier har full dekning mot admin-menyen. Landingssiden må bygges om når disse fem er ute.
...
## Siste sesjons-sammendrag (2. juli 2026 — lang økt)

Tema: grundig indekserings/SEO-gjennomgang, ny merke-vegg på `/produsent`, kiosk-fikser, skrivebords-opprydding, Wera-IG-varsling og Supabase-egress-fiks.

### 1. Indekserings/SEO-gjennomgang
- **robots.txt Googlebot-hullet er FIKSET live** (den tomme `User-agent: Googlebot / Disallow:`-blokka er borte, `/Search.aspx` disallowed, Sitemap-direktiv på plass). Bekreftelse: **0 parameter-URLer i GSC**. → junk-bøttene skal krympe; verifiser ~29. juli.
- **Sitemap 4970 URLer = 100% sunn** (opp 194 fra 4776 den 12. juni). Kannibalisering `/manufacturers/`+`/categories/` = 0. Legacy + parameter-URLer i GSC = 0.
- **Funn: alle 30+ produsent-landingssider mangler i sitemap** (`/wera`, `/snapon`, `/pelicase`, `/facom`, `/milwaukee`, `/hdfi`, `/om-oss`, `/aktuelt/*`). Kun 4 single-segment-sider er i sitemap (`/produkter`, `/aviation`, `/bransjer`, `/referanser`) + forsiden. **Ikke** en indekserings-feil — URL Inspection bekrefter `/wera` = «Submitted and indexed», PASS. Google fant dem via intern-lenking. Avklart som Multicase-begrensning (skjult avdeling) → won't-fix, lav reell påvirkning.
- **Største selvbetjente lever: 60 begravde kategorisider** (indeksert, men pos 30–90 med store visninger): `/produkter/fallsikring` 1798v@pos60, containere, arbeidsbord, tenger, verktøyvogner, hodelykt m.fl. Trenger produsent-mal-behandling (INTRO+FAQ+JSON-LD+catgrid+intern-lenking). *(Ikke startet — neste gang.)*
- Site-total: klikk +4.5%, visninger +5.6%. Facom recovered (pos 16.7 april → 8.9). **Video-hero vs image-hero: video 9.0 vs image 12.2, begge flate → ingen handling, sjekk lukket.**
- Deliverable: `~/Desktop/FT-indeksering-gjennomgang-2026-07-01.html`. Scripts: `_tmp-seo-coverage-audit.mjs`, `seo-health-check.mjs`, `_tmp-indexing-audit.mjs`, `_tmp-url-inspect.mjs`.

### 2. Merke-vegg på `/produsent` (+ full SEO)
- Søkbar merke-vegg som **auto-genereres fra `.ProductMenu`** (venstremenyen) — 55 merker, ett kort hver. Endrer du menyen → veggen oppdateres selv.
- **Logo auto-match på navn:** `/userfiles/image/Logo/{Navn med bindestrek}.svg`, med `OV`-override for unntak (Snapon→Snap-on, Gühring→Guehring, Völkel→Voelkel, Zweibrüder→Zweibruder, The Bone→the-bone.png) + `EXTRAS` for merker utenfor menyen (Halder). `onerror` → tekst-wordmark-fallback (aldri knekt bilde).
- **Stilene i `FosenTools.scss`** (scopet `#ft-brandwall`, ingen JS-CSS-injeksjon, ingen `!important` — SCSS vinner på spesifisitet). Overskrift sentrert + svart. Søk via event-delegering på `document` (capture) — overlever Multicase re-render.
- **Full produsent-side-SEO-standard:** title `Verktøymerker og leverandører | Fosen Tools` + meta (CMS), inline **BreadcrumbList** + **FAQPage** (5 spm), JS-generert **CollectionPage/ItemList**, FAQ-seksjon (`ftseo-faq`) + kontakt-CTA (`ft-contact-cta`). Verifisert live med Googlebot-UA. Deliverable: `~/Desktop/FT-merkevegg-KILDE.html` (SCSS + Kilde-HTML m/ kopi-knapper). Logoer for Halder/Picard/Zweibrüder samlet i `~/Desktop/FT-merkelogoer-til-opplasting/`.

### 3. Produktbeskrivelse-rydder
- Multicase WYSIWYG-editor legger igjen tomme div-er (`<br><div style=""><div style=""></div></div>`) + `style=""` + `&nbsp;` på slutten av beskrivelser → `.prod-text-content` lukkes ikke rent → **«Teknisk info» + «Lignende produkter» nøstes inn i Beskrivelse-fanen** i stedet for å være egne faner. Verktøy: `~/Desktop/FT-beskrivelse-rydder.html` (lim inn → rydd → kopier). Memory: `feedback_multicase_produkttab_cruft`.

### 4. Kiosk-skjerm musepeker
- Skjermene (`/skjerm/{token}` — Wera/Kasse/Milwaukee/HDFI) viste musepeker. Fikset i `custom-slide-renderer.tsx` (**`pointer-events:none`** på YouTube- + iframe-slides → musa faller gjennom cross-origin-iframen → `cursor:none` på underliggende element) + `/skjerm/[token]` (**transparent 1×1 cursor-bilde** som fallback for nettlesere som ignorerer `cursor:none`). **Wera fikset ✅.** Milwaukee = infoskjermen.no-spillerens egen OS-peker (utenfor nettsiden) → må skrus av i deres oppsett, web-kode kan ikke fjerne den.

### 5. Skrivebord ryddet
- 205 → 2 mapper: **`Fosen Tools/`** (alt FT + arkivmappene SKJERMBILDER/`_arkiv-jubileum-2026`/`_arbeidsfiler-arkiv`) + **`Andre prosjekter/`** (Hundo Hunter, Shadowoaths/Pact, TikTok Edit Maker, Diverse). Alt flyttet, ingenting slettet.

### 6. Wera-IG-varsling (ny — LIVE)
- **Offisiell IG Graph API «Business Discovery»** mot `@weratoolrebelsnorway` (FT sin IG er nå koblet, bruker-ID `17841456476093174`). Cron poller siste innlegg → nytt siden sist? → **ntfy.sh-push** til mobil (caption + IG-lenke). Baseline settes ved førstegangskjøring (ingen spam av gamle innlegg).
- Filer: migrasjon `024_social_watch.sql` (tabell), `src/lib/services/social-watch.ts`, `src/app/api/social-watch/route.ts`, `.github/workflows/social-watch.yml` (poll hvert 30. min, siden Vercel Hobby-cron ikke kan polle ofte). **ntfy-emne:** `ft-somewatch-cfaa9c481549` (i `.env.local` + Vercel `NTFY_TOPIC`).
- **Innebygd token-utløp-varsel:** pusher advarsel ~7 dager før Meta-tokenet utløper (31. aug). **Facebook:** ingen offisiell API for andre siders innlegg → IG dekker i praksis (Wera cross-poster). Verifisert prod-test 200 + GitHub Actions grønn. Memory: `feature_social_watch`.
- **Meta-token oppdatert** (var utløpt siden 25. juni — fikset også Meta-synken som hadde ligget nede). Utløper 31. aug → bytt til permanent System User-token før da (datostyrt påminnelse lagt inn).

### 7. Supabase Cached Egress-fiks
- Var **155% / 7,7 GB** av gratis-grensen mens Storage Size bare var 99 MB → **de samme brand-logoene servert tusenvis av ganger** av butikk-skjermene som reloadet **hvert 5. min**. Fikset: reload → **1×/dag kl 07:30** (288→1 sidelast/dag/skjerm), innholds-poll 30s→5min. Playlist-endringer vises fortsatt. Egress nullstilles **10. juli** (ny syklus); grace til 26. juli er engangs. Datostyrt sjekk ~13. juli. Memory: `feedback_supabase_egress_kiosk`.

### Commits (alt pushet til main)
`c56334d` (kiosk-cursor), `f28101b` (social-watch), `7f848cb` (egress-fiks). SEO-arbeidet (merke-vegg, produktbeskrivelser) ble limt direkte i Multicase (ikke i repoet).

---

## Siste sesjons-sammendrag (16. juni 2026 — jubileums-SoMe-poster + SEO-register)

**Tema:** Produsere daglige jubileums-poster (25-årsjubileum + proff-butikk-åpning 26. juni) + bygge SEO produkt-gap-register. Full produksjonssystem + status: se memory `project_jubileum_some_poster_produksjon`.

**Jubileums-poster (HTML→PNG via node+Playwright, maler i `~/Desktop/Jubileum-Claude-Design-RESULTAT/jubileum-poster/`):**
- **Milwaukee** (T-10) — PUBLISERT på IG, FB+LI scheduled kl 14. RED Evolution Heavy Duty Days-logo + Milwaukee-logo, «Milwaukee kommer til jubileet», «Hold av fredag 26. juni», innblikk i Milwaukee-avdelingen (full avduking på dagen), bredden M12/M18/MX FUEL/tilbehør/verneutstyr/håndverktøy/oppbevaring, «mye av produktene Milwaukee viser fram lagerfører vi», INGEN priser. 4:5 + 1:1 + captions + ALT. `~/Desktop/Jubileum-Milwaukee-FERDIG/`.
- **Wera** (T-9, 17. juni) — FERDIG. Foto/logo-forward: fullformat grønn Wera-vegg + STOR hvit Wera-logo (`wera-white-trim.png`) + «Nyheter og innovasjon innen håndverktøy» + stor «Fredag 26. juni»-dato-callout. Ingen produktliste (vet ikke hvilke nyheter). 4:5 + 1:1 + captions + ALT. `~/Desktop/Jubileum-Wera-FERDIG/`.
- **«Én uke igjen»** (T-7, fredag 19. juni) — 4:5 FERDIG+GODKJENT, **mangler 1:1 + captions + ALT (FORTSETT HER)**. Dristig sentrert grafikk-countdown: FT-rød bakgrunn, solstråle-vektor (repeating-conic-gradient) + konsentriske ringer, **stor sirkel-medaljong (gull-ring) med Factory Store-bilde** øverst, gigantisk «ÉN UKE IGJEN!», svart «Fredag 26. juni»-banner, chips: «Grilling fra kl. 11 · 6 leverandører + 2 spesielle gjester · Dagstilbud kun i butikken», «Åpent for alle, ingen påmelding». HTML: `T-7-en-uke.html`, PNG: `~/Desktop/Jubileum-EnUke-FERDIG/`.
- **4-generasjoner** (torsdag 18. juni) — PÅ VENT (navn fra pappa + landingsside).
- **Leverandør-wall** (Soudal/Picard/Halder/Zweibrüder — de uten egen butikk-sone) — bilde bygget (`~/Desktop/Jubileum-Leverandorer-FERDIG/`), captions+kvadrat gjenstår.

**Konvensjoner låst (ikke re-spør):** render via node+Playwright (file:// → fonts.ready → screenshot, 1080×1350 / 1080×1080, scale 2); formater 4:5 (IG) + 1:1 (FB+LI); brandrow ft-logo + jub-100 + jub-25 + rødt «Fredag 26. JUNI»-stempel; captions.html m/ kopi-knapper + ALT-seksjon; UTM `jubileum-2026-06-26`; bilder må lastes ned til `~/Downloads` som ekte filer. **Regler:** «på Brekstad», komma ikke tankestrek, INGEN priser, «åpner proff-butikk» (ikke «ny»), «Zweibrüder» (ü), dagstilbud KUN i butikken, «innblikk»=bildet/glimt, **6 leverandører + 2 spesielle gjester** (Red Bull/Tesla=gjester), hver post eget uttrykk. **Jubileumsinfo** (fosen-tools.no/jubileum): 10:00–16:00, grilling fra kl. 11, åpent for alle ingen påmelding, dagstilbud kun i butikken.

**SEO produkt-gap-register** (`~/Desktop/FT-SEO-produkt-gap-register.xlsx`): Oversikt + Undersider-byggeliste + ark per produsent. Ferdig: Leatherman (kjøp inn Free P4/P2/Signal), Milwaukee (on-page bekreftet OK via **GSC URL Inspection API** — den autoritative on-page-sjekken, IKKE curl; mulighet = merke×kategori-undersider, venter på Multicase), Pelicase (hard autoritets-konkurranse, ingen quick fix). Neste produsent: Wera eller Mitutoyo. Multicase-melding om undersider draftet, ikke sendt.

**Tidligere i samme økt (16. juni) — ferdig/lukket:**
- **Arne Frode-sitat-post (jubileum) PUBLISERT** — `~/Desktop/Jubileum-Arne-sitat-FERDIG/` (HDFI-nærbilde + sitat «I 25 år har vi utviklet og produsert driftseffektive og innovative løsninger…» + portrett-sirkel, 4:5 + 1:1 + captions + PDF).
- **71xx-postnummer e-postliste** renset til tab-delimitert e-postfil + audience-CSV for Mailchimp-import (`~/Desktop/jubileum-71xx-emails.txt` + `jubileum-71xx-audience.csv`) — for jubileums-e-post til 71xx-segmentet.
- **Wera-logo-fix i jubileum-event-slideshow** + slideshow-innholdsfikser (sømløs partner-marquee, «kun i butikken»-budskap på event-slide, logo-slider som ikke stopper) — committet `19de146` + `c10ecbd`, pushet til main.
- **Video-hero vs image-hero SEO-sjekk** (overdue datostyrt #5) kjørt via `scripts/video-vs-image-seo.mjs` — oppfølging av 16. mai-funn (video-hero -1.9 pos vs image-hero +1.6); banner/hero-type-effekt på rangering.
- **Facom SEO-gjenhenting** vurdert (om Facom klatrer tilbake mot tidligere pos; intern-lenking diskutert som mulig løft).
- **Leatherman SEO deep-dive** — bekreftet at `/leatherman` er sunn (H1 + 3 JSON-LD bekreftet via **Googlebot-UA**, ikke Mozilla — quirk #14). «Fallet» var produkt-/sortiment-gap (mangler Free P4/P2/Signal), ikke on-page → ledet direkte til SEO produkt-gap-registeret.

## Siste sesjons-sammendrag (15. juni 2026 — mandag: nyhetsbrev + brief-fixer + Meta-rapport)

Tema: ferdigstille tirsdags-nyhetsbrevet, rydde konverterings-/lead-definisjoner i møtebriefen, og lage Meta-annonse-rapport til mandagsmøtet.

### Tirsdags-nyhetsbrev (16. juni) — Knipex valgt + klart
- **Knipex-utkast** (`ba48a430…`) ble valgt og er publiseringsklart. Mitutoyo-utkast (`741fcb63…`) beholdt som alternativ.
- Bygg-script: `scripts/_tmp-build-knipex-nl.ts`. Preview: `~/Desktop/nyhetsbrev-16juni/preview-knipex.html` (+ `-entities.html` = HTML-entitet-kodet versjon for Mailchimp, æøå→`&#248;` osv.).
- **Levert som HTML på utklippstavle** (ikke pushet via API — Adrian limer inn i Mailchimp «Paste in code» + scheduler manuelt tirsdag 11:00). Subject «Tenger for fagfolk», preheader «Kombinasjonstang, rørtang og boltekutter fra Knipex…».
- **Midt-seksjon:** egendesignet VDE-grafikk (`scripts/_tmp-render-knipex-midt.mjs` → Supabase `social_assets/brand-assets/knipex-vde-midt.png`), CTA → VDE/1000V-filtrert Knipex-side.

### Nye nyhetsbrev-standarder (kode-håndhevet — gjelder ALLE fremtidige)
1. **Topp-badge = «NYHETSBREV»** som default for vanlige produsent-/produkt-nyhetsbrev (content-generator). Salgs-badges (% RABATT/KAMPANJE) beholdes ved tilbud.
2. **Pris vises KUN ved rabatt** — `formatPriceText()` returnerer tom streng uten rabatt. Ikke-rabatterte produkter: navn + art.nr + «Gå til produkt».
3. **FT-artikkelnummer** (Multicase `.prd-num-label`, f.eks. «F1880») på produktkort, IKKE produsentens modellnummer.
4. **Mobil-stacking:** produkt-`<td>` har nå `class="mceColumn"` → stacker 1-per-rad på mobil (var 3+2 og bittesmå). Desktop uendret.
5. **Tom pris-linje fjernes** (ikke min-height-glipe) når priceText er tom.

### Møtebrief-fix: begin_checkout ≠ lead
`begin_checkout` (påbegynt kasse) ble feilaktig talt som lead 4 steder. Ryddet i `weekly-brief`, `weekly-validation`, `google-ads/analysis`, `keyword-intelligence` → nå kun `form_submit`/`kontakt` = lead; `begin_checkout` = egen «kjøpsintensjon/pipeline». Brief har ny «Påbegynt kjøpsverdi (pipeline)»-post. Se memory `feedback_begin_checkout_ikke_lead.md`.

### Meta-annonse-rapport (mandagsmøte)
`scripts/_tmp-meta-report.mjs` → `~/Desktop/meta-resultater.html`. Viser betalt jubileums-annonse (3 202 nådd / 85,99 kr / CPM 19,70 / 29 % budsjett), UTM-trafikk (89 Meta-sesjoner, 7 fra annonsen), daglig side-rekkevidde (+575 % jubileums-surge), jubileums-post (organisk). Merknad: betalt spend/ROAS finnes ikke i appen (kun Ads Manager-CSV); annonse-objektiv = rekkevidde, så få nett-klikk er forventet (dagstilbud kun i butikken).

### 4-generasjoners-bilde mottatt 15. juni (jubileum / 100 år)
Adrian fikk tilsendt bilde av 4 generasjoner Pettersen fra faren (bakgrunn fjernet, PNG). **Personer fra venstre:**
1. **Adrian Pettersen** (meg / yngst)
2. **Arne Frode Pettersen** (far)
3. **Oddvar Pettersen**
4. ⚠️ **4. person ubekreftet** — Adrian trodde «Arne Pettersen» men er usikker. **MÅ verifiseres med faren før bruk** (jf. «aldri gjett navn»-regelen).
Brukes til jubileums-poster (generasjonsbilde / 100-årskonsernet — passer T-01 «generasjonsbilde» eller eget innlegg). Adrian må legge bildefila i `~/Downloads/` og verifisere 4. person før det settes inn + captions skrives.

### Fortsatt åpent (overført)
- **Jubileums-poster:** venter på foto av proff-butikk under arbeid → T-11 (`scripts/_tmp-render-claude-design.mjs`). Traction-utvalg klart i `~/Desktop/Jubileum-TRACTION-UTVALG/`. ⚠️ DAG-c-direkte + Konsept-curiosity ekskludert.
- **robots.txt-fiks til Multicase:** `~/Desktop/multicase-robots-txt-fix.html`.
- Kode-endringene (brief/leads, builder, content-generator) er i working tree — ikke committet ennå.
